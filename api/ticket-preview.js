...
export default async function handler(req,res){const id=randomUUID();try{
  ...
  const preview={guild:{id:DARK_SIDE_GUILD_ID,name:'The Dark Side'},stats:previewStats};
  if(!requested)return send(res,200,{premium:false,source:'preview',...preview,request_id:id},id);
  ...
  if(premium){
    try{
      ...
      return send(res,200,{premium:true,source:'selected',guild:{id:guild.id,name:guild.name},stats:await ticketStats(requested),preview,request_id:id},id);
    }catch(error){
      if(error instanceof HttpError&&(error.status===401||error.status===403)){
        return send(res,200,{premium:true,source:'preview',admin_refresh_required:true,admin_error:error.message,guild:preview.guild,stats:preview.stats,selected_guild_id:requested,request_id:id},id);
      }
      throw error;
    }
  }
  return send(res,200,{premium:false,source:'preview',...preview,selected_guild_id:requested,request_id:id},id);
