...
async function isPremium(guildId){if(!PUBLIC_SERVICE_KEY)return false;const rows=await rest(PUBLIC_SUPABASE_URL,PUBLIC_SERVICE_KEY,`premium_dashboard_guilds?select=guild_id&guild_id=eq.${guildId}&active=eq.true&limit=1`);return Boolean(rows?.length)}
...
export default async function handler(req,res){const id=randomUUID();try{
  ...
  if(premium){
    ...
    return send(res,200,{premium:true,source:'selected',guild:{id:guild.id,name:guild.name},stats:await ticketStats(guildId),request_id:id},id);
  }
  return send(res,200,{premium:false,source:'preview',guild:{id:DARK_SIDE_GUILD_ID,name:'The Dark Side'},stats:await ticketStats(DARK_SIDE_GUILD_ID),request_id:id},id);
