(() => {
  ...
  async function request(method='GET', body){const id=guildId(),provider=providerToken();if(!id||!provider)throw new Error('Private dashboard session is not ready.');const r=await fetch(`/api/private?guild_id=${encodeURIComponent(id)}`,{method,headers:{'X-Discord-Provider-Token':provider,...(body?{'Content-Type':'application/json'}:{})},body:body?JSON.stringify(body):undefined});const d=await r.json().catch(()=>({}));if(!r.ok)throw new Error(d.error||`Private dashboard failed (${r.status})`);return d;}
  function ensurePanel(){const area=document.getElementById('privateBuildArea');if(!area||document.getElementById('privateControlCentre'))return;const panel=document.createElement('section');panel.id='privateControlCentre';panel.className='private-control-centre';panel.innerHTML=`
    ...
    <div class="private-control-layout">
      ...
      <article class="private-control-card"><div class="private-control-title"><span>⛓</span><div><small>ECONOMY BRAND</small><h4>Make Bonds yours</h4></div></div><label><span>Currency name<small>Rename Bonds to whatever fits the server</small></span><input id="privateCurrencyName" maxlength="24" placeholder="Bonds"></label><label><span>Currency icon or emoji<small>Unicode emoji or Discord custom emoji markup</small></span><input id="privateCurrencyIcon" maxlength="100" placeholder="💰"></label><div class="currency-preview"><small>PREVIEW</small><b id="privateCurrencyPreview">💰 12,500 Bonds</b></div></article>
    </div>
